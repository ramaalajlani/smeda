...
// 6. Validate OpenAPI YAML via Symfony (Laravel vendor)
require $apiRoot . '/vendor/autoload.php';
$yamlRaw = file_get_contents($docsDir . '/openapi.yaml') ?: '';
try {
    $openapiDoc = \Symfony\Component\Yaml\Yaml::parse($yamlRaw);
    $okOpenApi = is_array($openapiDoc) && ($openapiDoc['openapi'] ?? '') === '3.1.0';
    logStep($results, 'OpenAPI YAML parse + version 3.1.0', $okOpenApi);
} catch (Throwable $e) {
    logStep($results, 'OpenAPI YAML parse', false, $e->getMessage());
    $openapiDoc = [];
}
...
$opIds = [];
$dupOpIds = 0;
foreach (($openapiDoc['paths'] ?? []) as $path => $methods) {
...
logStep($results, 'OpenAPI operations count', $oasOps > 0, (string) $oasOps);
...
foreach ($swaggerAssets as $name => $url) {
    $ctx = stream_context_create(['http' => ['timeout' => 60], 'ssl' => ['verify_peer' => true]]);
    $data = @file_get_contents($url, false, $ctx);
    if ($data === false) {
        logStep($results, 'Download swagger-ui ' . $name, false, $url);
        continue;
    }
    file_put_contents($swaggerDir . '/' . $name, $data);
