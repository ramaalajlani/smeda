...
<div class="tc-phone">
  ...
  <div class="tc-content">
    <div class="tc-scope is-course" style="margin-bottom:12px">
      <div class="tc-scope-txt"><i class="bi bi-person-workspace"></i> مساحة المدرب — الدورات المعيَّنة لك فقط (حضور ودرجات ومتدربون)</div>
    </div>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!ok) return;
  if (window.AppAuth.isCenterWorkspaceUser && AppAuth.isCenterWorkspaceUser()) {
    location.replace('center-app.php'); return;
  }
  ...
  try {
    ...
    if (!courses.length){
      box.innerHTML='<div class="tc-empty">لا توجد دورات معيَّنة لك حالياً</div>';
      return;
    }
    box.innerHTML = TC.searchBoxHtml('tcSearch','بحث عن دورة...') +
      ...
      return `<div class="tc-item" data-search="${E(hay)}">
        ...
        <div class="tc-item-card">
          ...
          <a class="tc-item-btn" href="center-course.php?id=${c.id}"><i class="bi bi-diagram-3"></i> فتح الدورة</a>
        </div>
      </div>`;
    }).join('')}</div>`;
